...
output = 'generated/dgg-REAL-ip-bans/'
...
banRE = '^\[[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2} UTC\] Bot: \w+ banned'
        ...
        for day in range(1, daysInMonth[monthIndex] + 1):
            ...
            if (r.status_code != 200):
                ...
                for line in lines:
                    ...
                    if (len(matches) == 1 and 'banned through' in line):
                        for ban in matches:
                            print(line)
                            username = ban[len('[1234-56-78 04:20:69 UTC] Bot: '):][:-len(' banned')]
...
for year in years:
    ...
    plt.title('REAL DGG bans in ' + str(year))
    plt.xlabel('REAL Bans')
    plt.ylabel('Usernames')
    plt.barh(topBans[year], list(map(bans[year].get, topBans[year])))
    plt.savefig(output + 'dggREALBans' + str(year) + '.png')
...
plt.rcParams.update({'font.size': 22})
plt.subplots_adjust(left=0.3)
plt.title('Total REAL DGG bans')
plt.xlabel('REAL Bans')
plt.ylabel('Usernames')
plt.barh(topTotalBans, list(map(totalBans.get, topTotalBans)))
plt.savefig(output + 'dggREALBansTotal.png')